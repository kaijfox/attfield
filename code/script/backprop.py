...
def main():
    ...
    if args.attn is not None:
        print("Loading attention model:", args.attn)
        kws = atts.load_cfg(args.attn_cfg)
        att_mods = atts.load_model(args.attn, **kws)
    else:
        print("No attention model.", args.attn)
        att_mods = {}
    ...
    # Assemble category metadata before flattening imgs and ys dictionaries
    ...
    # Flatten imgs and ys dictionaries
    ...
    if not args.no_back:
        ...
        outputs = None
    # Optionally output scores along classifiers' decision axes as well
    if args.regs is not None:
    ...
    # NOTE: later code assumes we'll still see images in expected order,
    # so no parallel processing is done on batches
...
def run_batch(
    ...
    fn_outputs, outputs, regs,
    cat_names, cat_ids, cat_img_ixs, cat_ns):
    """Run gradients calculations for a batch and add them
    to running totals."""
    
    if args.verbose: print(f"Running batch.")
    
    # Bypass pre-trained classifiers in CORnet
    decoder_bypass = {l: det.LayerBypass() for l in args.decoders}

    # Select images for the batch
