...
from lib.plot import diagrams
from lib.plot import normalization
...
class params:
    ...
    # --- Data inputs
    # RF ellipse fits
    dist_ells = paths.data('gauss/summ_base_ell.csv')
    focl_ells = [
        paths.data('gauss/summ_cts_gauss_b1.1_ell.csv'),
        paths.data('gauss/summ_cts_gauss_b2.0_ell.csv'),
        paths.data('gauss/summ_cts_gauss_b4.0_ell.csv'),
        paths.data('gauss/summ_cts_gauss_b11.0_ell.csv'),
    ]
    # Effective gain
    sgain_focl = [
        paths.data('gauss/lenc_task_gauss_b1.1.h5.sgain.npz'),
        paths.data('gauss/lenc_task_gauss_b2.0.h5.sgain.npz'),
        paths.data('gauss/lenc_task_gauss_b4.0.h5.sgain.npz'),
        paths.data('gauss/lenc_task_gauss_b11.0.h5.sgain.npz'),
    ]
    # Raw gradients
    grads_dist = paths.data('gauss/rfs_base.h5')
    grads_focl = paths.data('gauss/rfs_cts_gauss_beta_11.0.h5')
    # Effective gains after nonlinear normalization
    norm_betas = np.arange(1.01, 1.10, 0.01)
    norm_exponents = np.arange(1, 4, 0.2)
    norm_base = paths.data('gauss/enc_l1_base.h5.norm.sd.npz')
    norm_att = [
        paths.data('gauss/enc_l1_gauss_b{beta:.2f}.h5.norm.sgain.npz')
        for beta in norm_betas]
    norm_layers = ['0.1.3']
    norm_att_loc = (.25, .25)
...
# load 1x1 readout scores
scores_1x1 = np.load(params.scores_1x1)

# load nonlinear normalization data
norm_data = normalization.normalization_data(
...
mappable = normalization.normalization_heatmap(
    norm_data, params.norm_betas, params.norm_exponents,
...
normalized_gain = norm_data[-1][params.norm_layers[0]][len(params.norm_exponents)-1]
