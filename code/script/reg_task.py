"""
Generates an HDF5 archive containing behavioral results of a neural network
observer model on a given set of stimuli, treated as a category-detection task.

For each category `CAT` in the provided detection task, the archive will
contain datasets
    CAT_y : int8 [0 | 1], shape (n_imgs,)
        presence or absence of the target category in each stimulus
    CAT_ix : uint32, shape (n_imgs,)
        index of each stimulus in the input HDF5 archive
    CAT_fn : float32, shape (n_imgs,)
        score of the stimulus along the decision axis of the provided linear
        network head for the category"""
...
def run_batch(
    ...
    fn_ds, reg):
    """Run observer model and linear classifier on a batch of stimuli."""
    ...
    # Extract layer 4 encodings and pass through classifier
...
def main():
    ...
    for c in four_task.cats:
        ...
        # Sort stimulus indices into batches
        ...
        # NOTE: later code assumes we'll still see images in expected order and
        # does not respect the `CAT_ix` dataset, so batches are not processed
        # in parallel
