"""
Defines CORnet_Z with an added identity layer at the input functioning as a
retina (hence the postfix `R`), which will be reprenseted as a layer in
the NetworkManager for which unit activations may be computed with respect to,
and to which attention `LayerMod`s may be applied.
"""
...
def get_model(weight_root = paths.data.join("models")):
    from lib.experiment.cornet import load_cornet
