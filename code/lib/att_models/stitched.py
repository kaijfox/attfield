...
Implementation of the gain-only attention model as used in `fig-flat`. Applies
a flat gain field over target quadrant with gain-independent recetive field
shifts at the boundary by splitting all convolutions into four separate
operations using the `Cutter` mod -- effectively zeroing connections across
the quadrant boundary if the convolution is treated as a fully-connected layer
-- and applying uniform gain to the target quadrant using the `beta` parameter
of one of the `Cutter`s. In addition, `Merge objects` should be applied to the
layer after every convolution to ensure pooling layers function as expected
across the quadrant boundaries.
