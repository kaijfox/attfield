...
Implementation of the shrinkage-only model used in `fig-shrink`. Uses a
functional approximation of the map from RF distance to the attention
locus to RF size change resulting from `beta=11` Gaussian gain to detemine
desired shrinkage of receptive fields given their location in a feature
map, and shrinks the corresponding convolution sencil (see `deformed_conv`)
accordingly.
