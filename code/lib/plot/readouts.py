"""
Functions for plotting behavioral results of readout-manipulation conditions
as in `fig-reconstruct` and its supplements, and for examining alignment of
encodings to a readout layer's decision axis.

### Classes
- `readout_data` - Container object for precomputed readout data of activations
    focal and distributed conditions passed through weights with and without
    a propagated gain map added/corrected for, and for the propagated gain map
    itself.

### Functions
- `load_logregs` - Wrapper for `lib.experiment.detection_task.load_logregs`
- `r2_map` - Compute alignment to a decision axis for units at each location
    in a feature map.
- `gain_map` - Plot the propagated gain map as a heatmap to the given axis.
- `auc2d` - Convert AUC to d' using a standard normality assumption.
- `d2auc` - Convert d' to AUC using a standard normality assumption.
- `jtr` - Apply jitter to an array for plotting.
- `reconstructed_bhv_auc` - Measure AUC in distributed, focal, multiplied,
    and divided conditions from a `readout_data` instance.
- `reconstructed_bhv` - Compute AUCs from a `readout_data` instance and plot.
- `reconstructed_bhv_plot` - Plot behavioral performance in distributed, focal,
    multiplied, and divided conditions to the given axis. 
