"""
Plotting functions for data mapped against distance from locus of attention,
usually with LOESS summary lines.

### Functions
- `rf_data` - Load receptive field data from a distributed condition and
    multiple focal conditions, and compute resulting size and location
    changes.
- `rf_file_iterator` - Generate an iterator over focal condition files from
    dataframe returned by `rf_data`.
- `rf_layer_iterator` - Generate an iterator over layers from dataframe
    returned by `rf_data`.
- `gain_data` - Load gain data from `.sgain.npz` files generated by `act_dists`
    given a set of units.
- `gain_file_iterator` - Generate an iterator over focal condition files from
    results of `gain_data`.
- `gain_layer_iterator` - Generate an iterator over layers from gain data.
- `lineplot` - Plot data in a file or layer iterator to an axis against
    distance of receptive field from the locus of attention.
- `mini_lineplot` - Plot a miniature version of `lineplot` across several axes.
