...
- `labels` - Apply axis labels.
- `minigrid_labels` - Apply axis labels to a set of breakout panels.
- `axis_expand` - Move borders of an axis.
- `colorbar` - Create a colorbar beside given axis.
- `legend_transform_data` - Generate a matplotlib transform for placing a legend
- `legend` - Add a text and color legend to a plot
- `line_legend` - Add a text, color, and line sample legend to a plot
- `mean_ci` - Compute boostrapped confidence interval.
- `mean_ci_table` - Compute a table of boostrapped condifence intervals comparing
    several conditions.
- `binned_mean_line` - Compute an average line for data by binning.
- `running_mean_line` - Compute an average line for data using a moving average.
- `loess` - Compute a LOESS curve to summarize data.
- `expand` - Merge points in a scatterplot to have uniform opacity in overlaps.
