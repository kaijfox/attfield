...
Plotting functions for human and network behavioral data, as well as functions
for working with lists of LaTeX confidence interval commands.

### Functions
- `auc2d` - Convert AUC to d' using a normality assumption.
- `d2auc` - Convert d' to AUC using a normality assumption.
- `log_psyc` - Evalutate a logarithmic psychometric curve.
- `log_psyc_inv` - Evalutate inverse of a logarithmic psychometric curve.
- `human_bhv_data` - Load human-experiment data from a CSV file.
- `get_dprime` - Calculate d' from human experiment data
- `human_bhv_plot` - Plot behavioral results from the human psychophysics
    experiment.
- `bhv_data` - Load network observer model behavioral data from an HDF5 archive
- `bhv_plot` - Plot nerwork behavioral data across multiple models and conditions
- `update_ci_text` - Update confidence interval command in a LaTeX script
- `ci_text` - Generate a tuple representation of confidence interval data.
- `group_ci_text` - Extract partiucular confidence intervals from a dataframe of
    bootstrapping results in multiple conditions.

"""

from lib.plot import util
import lib.plot.kwargs as default_pkws
