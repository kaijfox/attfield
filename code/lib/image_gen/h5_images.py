...
Provider for images directly as stored in an HDF5 archive according
to category.

An image provider is a python script defining a `generate_set` function
in its globals that returns a tuple of
- Image group metadata (groups are always by category here), as a list
    of tuples containing the group label (category name) and a dictionary
    of associated metadata (here always an array of ground truth labels)
- Function returning an iterator over image groups, yielding a tensor for
    each group.
