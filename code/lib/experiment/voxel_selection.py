...
Tools for identifying and working with units / voxels in a torch model.

### Functions
- `deep_repr` - String conversion for nested collections of tensors and modules
- `deep_cat` - Concatenation for nested collections of tensors
- `random_voxels` - Uniform random sampling of voxels / units from a computed
    NetworkManager instance.
- `random_voxels_for_model` - Uniform random sampling of voxels / units from a
    torch model on given inputs.
- `load_units_from_csv` - Load serialized collection of units from a file.

### Classes
- `VoxelIndex` - Collection of methods to manipulate a set of voxels / units
    across multiple layers of a model, as well as tools for accessing activation
    of those units and generating masks for backpropagation of their activation
    derivatives.
...
class VoxelIndex():
    ...
    def index_into_batch(self, batch, i_vox):
        ...
        If `i_vox` is None, get the timeseries for each voxel in the
        VoxelIndex -- returns an array of shape (n_batch, n_vox).
        """
        if i_vox is not None:
            ret = batch
            for dim in self._idx:
                ret = ret[:, dim[i_vox], ...]
            return ret
        else:
            batch_slice = slice(0, len(batch))
            return batch.__getitem__((batch_slice,) + tuple(self._idx))
    ...
    @staticmethod
    def serialize(voxels):
        ret = {}
        for l in voxels:
            lstr = '.'.join(str(i) for i in l)
            ret[l] = [lstr + ';' + v for v in voxels[l]._serialize()]
        return ret
...
def load_units_from_csv(filename, col = 'unit'):
    ...
    return VoxelIndex.from_serial(df[col])