"""
Defines tools for managing the execution of a torch `nn.Module`. In
particular, the NetworkManager class assigns unique indices to
intermediate tensors, saves them, and patching of a module's
structure according to a dictionary of `LayerMod`s - which may transform
the inputs and outputs of each executed sub-module producing an
intermediate tensor.

### Classes
- `LayerMod` - An abstract class with no functionality of its own,
    outlining the functionality of the `pre_layer` and `post_layer`
    methods, which operate on the inputs and outputs of `nn.Module`s.
- `NetworkManager` - A class that wraps an execution of an `nn.Module`,
    preserving intermediate computed layers, and injecting `LayerMod`s.
- `NullManager` - A no-op class for unmanaged execution of networks.


### Functions
- `mod_merge` - Combine `LayerMod`s or dictionaries of `LayerMod`s.
- `summstr` - Recursive helper function for printing summaries of
    the managed execution of an `nn.Module`.
- `patch` - Monkeypatching function that attaches hooks to internal
    torch functions.
