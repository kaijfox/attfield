...
Tools for running four-way detection task on the neural network observer model.


## Classifier layers

### Classes
- `LayerBypass` - A LayerMod (see `network_manager.py`) to bypass existing
    clasifier layers in a model.
- `ClassifierMod` - A LayerMod adding pre-trained linear prediction layers.

### Functions
- `load_logregs` - Load a dictionary of sklearn or ClassifierMod linear layers
    from an NPZ file.
- `save_logregs` - Save a dictionary of sklearn or ClassifierMod linear layers
    to an NPZ file.
- `multi_decision` - Run the decision functions of a dictionary of linear layers.
- `by_cat` - Run a given function for each key in a dictionary.
- `score_logregs` - Compute model encodings and score a set of prediction layers
    running on those encodings.
- `score_logregs_precomputed` - Score a set of prediction layers based on
    precomputed model encodings.
- `model_encodings` - Compute model encodings on a set of images, bypassing its
    decoder layers from training. Optionally use a set of additional LayerMods
    such as attention models.
- `fit_logregs` - Compute model encodings as in `model_encodings` and train 
    linear prediction layers to replace the model's linear decoder layers
    from training.
...
### Classes
- `IsolatedObjectDetectionTask` - Manager for a set of stimuli in an isolated
    object present/absent task, using an `index.csv` as set up in `stimuli.md`.
- `FourWayObjectDetectionTask` - Manager for composite images in an object
    present/absent task based on a four-way grid, using an `index.csv` file.
- `DistilledDetectionTask` - Manager for task stimuli rendered to an HDF5
    archive.
- `FakeDetectionTask` - Manager for random noise stimuli to imiatate a true
    task during testing.

### Functions
- `generate_dataset` - Render a set of images in an `index.csv` to an HDF5
    archive for quick batched reading.
- `cache_iso_task` - Render stimuli from an IsolatedObjectDetectionTask based
    on the given set of imagenet images to an HDF5 archive.
- `cache_four_task` - Render stimuli from an FourWayObjectDetectionTask based
    on the given set of imagenet images to an HDF5 archive.
...
from lib.experiment import video_gen
...
class IsolatedObjectDetectionTask():
    ...
    def _load_set(self, cat, n, read_order,
                  shuffle = False):
        '''
