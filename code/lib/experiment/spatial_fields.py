...
Wrapper object for scipy's `LinearNDInterpolator`, applying it to 2 dimensional
vector fields, and KDTree-based interpolator for interpolation on a nonuniform
grid as is performed in the `quiverplot` function of `lib.plot.quivers`.

### Classes
- `TreeField` - The aforementioned `KDTree`-based interpolator.
- `LinearField` - The aforementioned wrapper.
...
class TreeField(object):
    ...
    def load(fname):
        with h5py.File(fname, 'r') as dat:
            ...
            return TreeField(tree, dat['r_data'][...], dat['c_data'][...])
